...
import { RESPONSE_FALSE_PICTURE_NOT_FOUND, RESPONSE_FALSE_USER_NOT_FOUND, RESPONSE_TRUE } from '../constant.response';
import { uploadPictureFirebase } from '../uploadFirebase';
import { getUserByUid } from '../user/user.model';
import { getAllPicture, getPictureByPid, getPictureByUid, getPictureRandom, insert } from './picture.model';
        ...
        if (result.length === NOT_FOUND) {
            ...
            if (req.file?.originalname === UNDEFINED) {
                ...
                insert(uid,downloadURL,(err:any,result:any)=>{
                    res.json({...RESPONSE_TRUE,url: downloadURL, 
...
export const randomPicture = (req:Request,res:Response) => {
